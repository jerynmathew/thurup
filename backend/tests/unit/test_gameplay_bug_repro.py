...
async def test_user_reported_gameplay_bug():
    ...
    if session.last_trick:
        ...
        # Jack = 3 points, 9 = 2 points, K = 0, Q = 0
        ...
        pytest.fail("No last_trick recorded after completing the trick!")
...
async def test_trump_ranking_all_combinations():
    ...
    for rank1, rank2, expected_winner_rank in trump_battles:
        ...
        await session.play_card(seat=1, cmd=PlayCardCmd(card_id=card2.uid))
        ...
        if session.last_trick:
            ...
            pytest.fail("No last_trick recorded!")
